...
```
     ...
     └────────────────────┘        └──────┬───────┘
                           ...
                           │ Wikidata, Gutenberg, LibriVox │
                           │ / Internet Archive, Wikisource│
                           │ Gallica;                      │
                           │ library catalogues: BnF, DNB, │
                           │ K10plus, LoC, LIBRIS, BN (PL),│
                           │ NDL (JP), Melinda, swisscovery│
                           └───────────────────────────────┘
```

Sources fall into two kinds, and the split is load-bearing rather than descriptive.
**Discovery** sources answer "does this book exist and what is it called" — the first group above.
**Enrichment** sources answer "what printings of it are there", and the library catalogues are only
ever that: a catalogue record is a _translation_, so discovering a Russian novel through the BnF
would file it under its French title and call French its original language. Enrichment is handed a
work id instead (`attachToWorkId`) and contributes editions to a book somebody else has already
identified. The two lists live in `apps/worker/src/composition-root.ts`; a catalogue in the
discovery list is a review error.

**A catalogue is always asked about the book _and_ its author, never the author alone.** The SRU
catalogues get every query word at once, so their own index does the narrowing. The two whose APIs
have no all-fields index — the Polish National Library and the NDL — pair an author field with a
title field instead (`splitQueryHalves`, `packages/infrastructure/src/providers/catalog-record.ts`).
This is not a refinement: asked by author alone, a catalogue answers with that author's entire
shelf and with everyone who merely shares a word with their name, and all of it gets attached to
one book. Asking the Polish library for `author=Lewis` on behalf of _Alice's Adventures in
Wonderland_ attached _Liar's Poker_ by Michael Lewis and _The Magician's Nephew_ by C. S. Lewis to
Carroll's novel — the author check passed them, because their author really is called Lewis.

The catalogues are read as **MARC21** wherever they offer it (`packages/infrastructure/src/providers/marc-record.ts`).
Dublin Core, which is the easier format and what the first two were built against, has no element
for the edition statement, no relator code for a translator and no language-of-original — so
through it a signed limited printing and the twelfth reprint are the same record. That is why rare
and collector's editions were invisible even for books already in the database.
...
| `edition`         | `id`, `work_id`, `title`, `language`, `translator`, `translated_from`, `publisher`, `year`, `isbn13`, `edition_statement`, `natural_key` | `natural_key` = ISBN-13 if present; otherwise hash(work_id, language, publisher, year, norm(title)). `translated_from` — the source language for this edition; per Phase 0 data it is populated more often than `translator` (16.4% vs 12.2% of editions) — a standalone "this is a translation" signal, not derived from the translator. `edition_statement` — how this printing differs from the others in the cataloguer's own words ("First edition", "Limited ed., signed", "Izd. 2-e, ispr. i dop."), taken from MARC `250`; deliberately **not** in the natural key, since two catalogues word it differently or omit it and keying on it would split one printing into several. The edition's source comes via `external_ref`; no dedicated field on the record itself is needed |
...
**A book is enriched twice: once when it is found, and once a night thereafter.** `ProcessBackfillJob`
runs the enrichment sources at discovery; the `RefreshStaleWorks` cron runs the same list again over
stale works, handing each one `attachToWorkId` so a catalogue can only add editions to the book it
was asked about and never invent a second one under a translated title. Without the nightly half, a
source added today would be asked about every book found from today onward and about none of the
books already in the database — which is what left «Метро 2034» at zero editions while seven
catalogues held it. The cost is one job per source per stale work, and `REFRESH_BATCH_SIZE` is the
lever on it.
...
POST /api/sync/:source  ──► EnqueueSourceSync ──► BullMQ (jobId = source-target-bucket)
                ...
                (discovery + enrichment, the latter
                 always with attachToWorkId)             │
                                                        ▼
                                              SyncWorkFromSource
                                                        │
                    ┌───────────────────────────────────┼───────────────────────────┐
