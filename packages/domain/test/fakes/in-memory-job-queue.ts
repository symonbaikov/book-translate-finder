import type { EnqueueOptions, JobPriority, JobQueuePort } from '../../src/ports/job-queue.port.js';

export class InMemoryJobQueue implements JobQueuePort {
  readonly enqueued: { jobId: string; payload: unknown; priority: JobPriority }[] = [];
  ...
  async enqueue(jobId: string, payload: unknown, options?: EnqueueOptions): Promise<void> {
    ...
    this.enqueued.push({ jobId, payload, priority: options?.priority ?? 'interactive' });
  }
}
