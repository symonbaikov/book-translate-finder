#!/usr/bin/env python3
"""Build the spike's EPUB fixture.

Deliberately hand-made rather than downloaded: the spike asks whether foliate-js can *measure* a
document inside an opaque origin, so the fixture has to be long enough to paginate into several
columns (a one-screen book would answer "the page did not turn" for the wrong reason) and small
enough to read in a diff. The text is Lorem-ipsum-shaped filler, not a real work.

Usage: python3 make-epub.py   →   spike.epub next to this file
"""
...
HOSTILE_SCRIPT = """
  try { fetch('http://localhost:3200/__beacon?via=inline-script'); } catch (e) {}
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
  ...
  <body>
    ...
    <img src="x-does-not-exist.png" onerror="fetch('http://localhost:3200/__beacon?via=img-onerror')" alt="" />
    <form action="http://localhost:3200/__beacon?via=form" method="get"><input name="a" value="b" /></form>
%s
  </body>
</html>
"""
...
EVIL_JS = """
try { fetch('http://localhost:3200/__beacon?via=external-script'); } catch (e) {}
